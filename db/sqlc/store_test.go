...
func TestTransferTx(t *testing.T) {
	...
	// crea due account randomly
	account1 := createRandomAccount(t)
	account2 := createRandomAccount(t)

	fmt.Println(">> before:", account1.Balance, account2.Balance)

	// esegue 5 transazioni concorrenti in cui trasferisce da account1 a account2
	// un amount random tra 1 e 10
	n := 5
	amount := int64(10)
	...
	for i := 0; i < n; i++ {
		go func() {
			...
			// invia i risultati e gli errori via channel
			errs <- err
			results <- result
		}()
	}
	...
	// qui, preleva i risultati e gli errori e li verifica
	for i := 0; i < n; i++ {
		err := <-errs
